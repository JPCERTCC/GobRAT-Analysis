...
class ConfigVar:
    ...
    the byte sequences created match.
    """
    matchNumber = 6
    ...
    """
    LessthanSize_MB = 10
    GreatorthanSize_MB = 2

    """
    Output filename
    """
    YaraFileName = "malware_GobRAT.yara"

    """
    Other
    """
    Other_RuleName = "GobRAT_arch"
    Other_Description = "Hunting GobRAT malware"
    Other_Auther = "JPCERT/CC Incident Response Group"
    Other_Usage = "Hunting"
...
class YaraStrings:
    ...
    def get_all_function_info(self):
        for segea in Segments():
            for funcea in Functions(segea, get_segm_end(segea)):
                ...
                #if ((functionName.startswith("main")) == True) or ((functionName.startswith("aaa_com_bbb")) == True):
                if ((functionName.startswith("aaa.com_bbb")) == True):
    ...
    def print_main_list(self):
        ...
        ### -- Following is each data structure -- 
        ### function_all_info = [
        ###     ['0x804dce9', '.term_proc',   ### Function-address, Function-name ### 
        ###         [                           ### Convert-flag, Bytes-length, Instruction ### 
        ###             [0, 3, '83 EC 0C ', 'sub     esp, 0Ch', 'sub     esp, 0Ch'],
        ###             [1, 5, 'E8 BF A3 FF FF ', 'call    sub_80480B0', 'call    NADDR'],
        ###             [0, 3, '83 C4 0C ', 'add     esp, 0Ch', 'add     esp, 0Ch'],
        ###             [0, 1, 'C3 ', 'retn', 'retn']
        ###         ]
        ###     ], ...
        ### ]        
        ### extracted_most_long_inst = [ [ Function-Number, max-Instruction-startPoint, max-Instruction-length ], ... ]
        ### yara_bytes = [ [ Function-Address, FunctionBytes, FunctionInstructions ], ... ]
...
class YaraCondition():
    ...
    con_filesize2 = sp8+ 'and (filesize > ' + str(ConfigVar.GreatorthanSize_MB) + 'MB)\n'
    ...
    def generate_yara_condition(self): 
        if self.file_format == "ELF":
            ...
            self.yara_condition += self.con_filesize2
...
class YaraGeneGobRAT_Plugin(idaapi.plugin_t):
    flags = idaapi.PLUGIN_DRAW
    comment = "YaraGenelate GobRAT for Multi Arch."
    help = "YaraGeneGobRAT"
    wanted_name = "YaraGeneGobRAT"
